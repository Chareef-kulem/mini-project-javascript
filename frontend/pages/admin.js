...
const SWR1 = () => {
        ...
        if (animas && animas.length)
            return animas.map((anima, index) =>
                <li className={styles.listItem} key={index}>
                    <h6>Legend Number:{(anima) ? anima.id : 0}</h6>
                    <img src={newja(anima.imageurl)} width="500" height="333"></img>
                    <h6>Legend Name:{(anima) ? anima.name : '-'}</h6>
                    <h6>Position:{(anima) ? anima.style : 0}</h6>
                    <h6>Age:{(anima) ? anima.like : 0}</h6>
    ...
    return (<div className={styles.container} >
        ...
        Legend Name:<input type="text" onChange={(e) => setName(e.target.value)} /> <br />
        Position:<input type="text" onChange={(e) => setStyle(e.target.value)} /> <br />
        Age:<input type="number" onChange={(e) => setLike(e.target.value)} /> <br />
                
            <button className={styles.byttonadd} onClick={() => handleCreate(name, style, like, reviews, score, imageurl)}>Add New Legend</button>
